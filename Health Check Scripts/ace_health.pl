...
chomp $row;
	if ($row =~ /New\sSection/) {
		...
		@array[$j] = "$date,ACE IP,$row,,,";
		#print "@array[$j]\n";
		$ip = $row;
		$j++;
	}
	elsif ($k == 2 && $row =~ /\d{4}-\d{2}-\d{2}/) {
		@array[$j] = "$date,ACE Date,$ip,,,";
		#print "@array[$j]\n";
		$j++;
	}
	elsif ($k == 3 && $row =~ /\d{1,2}.\d.\d/) {
		@array[$j] = "$date,ACE Buildstamp,$ip,$row,,";
		#print "@array[$j]\n";
		$j++;
	}
      ...
      if ($row =~ /ok/i) {
			$row =~ s/\W+/ /g;
			@array[$j] = "$date,ACE Raid,$ip,$row,OK,";
		}
		elsif ($row =~ /fail/i) {
			@array[$j] = "$date,ACE Raid,$ip,$row,Fail,";
		}
		#print "@array[$j]\n";
      $j++;
   }
#Testing the megaraid output
		...
		if ($row =~ /Enclosure Device ID/) {
			...
			#print "$status\n";
			if ($status =~ /Spun Up/) {
		      @array[$j] = "$date,ACE Raid,$ip,[$enclosure:$device]$status,OK,";
			}
			else {
				@array[$j] = "$date,ACE Raid,$ip,[$enclosure:$device]$status,Bad,";
			}
   	   #print "@array[$j]\n";
      	$j++;
...
#End testing megaraid output
		...
		if ($pctuse <= 30) {
			@array[$j] = "$date,ACE Disk,$ip,$row,good,";
		}
		elsif ($pctuse > 30 && $pctuse < 80) {
			@array[$j] = "$date,ACE Disk,$ip,$row,average,";
		}
		elsif ($pctuse > 80) {
			@array[$j] = "$date,ACE Disk,$ip,$row,high,";
		}
		#print "@array[$j]\n";
      $j++;
   }
		...
		if ($row >= 70) {
			@array[$j] = "$date,ACE CPU,$ip,The average CPU utilization is $row,high,";
		}
		elsif ($row < 70) {
			@array[$j] = "$date,ACE CPU,$ip,The average CPU utilization is $row,OK,";
		}
      #print "@array[$j]\n";
      $j++;
   }
		...
		if ($row >= 70) {
         @array[$j] = "$date,ACE Disk,$ip,The average disk utilization is $row,high,";
      }
      elsif ($row < 70) {
         @array[$j] = "$date,ACE Disk,$ip,The average disk utilization is $row,OK,";
      }
      #print "@array[$j]\n";
      $j++;
   }
      ...
      $msg = $2;
		...
		$dsdate =~ s/\s//;
		#print"$dsdate\n$msg\n";
      @array[$j] = "$date,ACE Red Flag,$ip,$dsdate $msg,,";
		$j++;
   }
$i++;
}
